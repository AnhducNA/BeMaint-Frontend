...
const SidebarItem = ({ item, pageName, setPageName, onAddTab }: any) => {
  const handleClick = () => {
    ...
    if (item.route !== "#") {
      onAddTab({ label: item.label, route: item.route });
    }
  };
    ...
    <>
      <li>
        ...
        {item.children && (
          ...
          >
            <SidebarDropdown item={item.children} onAddTab={onAddTab} />
          </div>
        )}
      </li>
    </>
